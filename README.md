...
- 상황
  - 차선 중앙에서 벗어난 정도를 조향 보정에 반영하는 기능을 구현했으나, 보정을 켠
    8/6 주행에서 차가 일관되게 왼쪽으로 쏠림. 시연은 보정을 끈 상태로 진행함.
- 원인 분석
  - 차선 경계 쌍이 한 프레임에 함께 잡혀야 중심을 계산할 수 있어 유효 프레임이 적었음
    (주행 데이터 기준 0.3\~39%)
  - 이를 보완하려고 넣은 편측 추정(한쪽 경계 위치 + 차선 반폭 가정)은, 잡히는 경계가
    주로 같은 쪽이라 가정이 어긋나면 오차가 한 방향으로 누적됨. 부호 반전 가능성도
    있었으나 어느 쪽인지 확정하지 못함.
  - 계산값이 맞는지 확인할 방법이 실제 주행뿐이어서, 주행 시간이 부족한 시연 직전에는
    원인 확정 자체가 불가능했음.
- 개선 방향
  - 정지 상태에서 차를 알려진 위치에 두고 계산값과 실측 이탈량의 부호·크기를 대조하는
    캘리브레이션 절차를 먼저 만들었다면, 주행 없이 원인을 확정하고 보정을 검증된
    상태로 켤 수 있었을 것.

**2. 버드아이 영상의 세로 방향 캘리브레이션 부재**

- 상황
  - 버드아이 영상에서 가로 방향은 축척(250px/m)으로 픽셀을 미터로 바꿀 수 있었지만,
    세로 방향(차량 전방 거리)은 화면 행과 실거리의 대응표가 없었음.
  - 그래서 회전 트리거를 "전방 몇 cm" 대신 화면 높이 비율(`near_row_frac`)로 정의함.
    동작은 했지만 값의 물리적 의미를 알 수 없어 튜닝이 시행착오에 의존했고, 카메라
    각도가 바뀌면 값이 통째로 무효가 되는 구조였음.
- 원인 분석
  - 주행 로그에서 행별 실거리를 역산하는 도구를 만들어 2회 시도했으나 실패.
    역산에 필요한 정상 주행 구간의 위치·방위 로그가 없었던 것이 원인
    (당시 위치 로그는 경고 메시지에만 부산물로 남았고, 이후 정기 로그를 추가함).
- 개선 방향
  - 정지 상태에서 알려진 거리(전방 20/40/60cm 등)에 표식을 두고 화면 행과 실거리의
    대응을 직접 측정했다면, 트리거를 물리 단위로 정의해 튜닝 근거를 명확히 할 수
    있었을 것.

**3. 회전 개루프의 개시 위치 민감성**

- 상황
  - 회전 구간은 GPS 품질이 가장 나빠, 회전을 고정 조향각 + 호 길이 종료 방식(개루프)으로
    전환해 완주함. 대신 도는 동안 보정이 없어, 회전을 시작하는 위치의 오차가 그대로
    착지(회전 후 도착 지점) 오차가 됨.
- 원인 분석
  - 완주 로그 분석에서 개시 위치가 착지 결과를 결정함을 확인 (`14->19` 회전).
  - 착지 실패가 잦던 `20->3` 우회전은 회전 자체를 고치지 못하고 정차점을 옆 차선으로
    옮겨 노선에서 제거하는 방식으로 회피함. 목적지가 늘어나면 다른 회전에서 재발할 수
    있는 구조.
- 개선 방향
  - 트리거를 화면 비율 대신 물리 단위로 정의(위 2번)했다면 개시 위치 오차를 줄일 수
    있었을 것.
  - 회전 구간에 카메라를 추가해 측위 품질을 확보했다면 회전을 다시 피드백 제어로
    되돌릴 수 있었을 것.

**4. 측위 오차를 차량 측 보정으로만 흡수**

- 상황
  - 측위는 3m 변 중앙에 세운 카메라 2대가 차량 마커를 인식해 좌표를 측량하는 구조.
    서버가 보고하는 좌표가 실제와 어긋났는데, 축척성 오차(x 3.0% / y 4.7%)에 더해
    위치마다 크기가 다른 불규칙 오차가 섞여 있었음.
- 대응
  - 규칙적인 성분은 실측이 확실한 정지선 4곳에서 (서버 좌표, 실제 좌표) 쌍을 얻어
    아핀 변환을 적합, 차량의 좌표 수신부 한 곳에서 보정함 (잔차 평균 3.5cm).
  - 불규칙 성분은 보정이 불가능해 점프 검사·터널 모드 같은 방어 로직으로 감당함.
- 원인 분석
  - 카메라 2대가 5×3m 트랙 전체를 나눠 커버하는 구조라, 카메라에서 먼 지점일수록
    픽셀 분해능·원근 왜곡으로 오차가 커짐.
  - 두 카메라의 시야 경계에서 좌표 불일치 발생 (같은 지점을 x축으로 32cm 다르게
    보고한 사례를 실측으로 확인).
  - 특정 구역은 마커 인식 유실이 잦아 별도 대응(터널 모드)이 필요했음.
- 개선 방향
  - 카메라를 추가 배치해 각 지점을 더 가까운 카메라가 담당하게 했다면, 불규칙 오차와
    유실 구역을 함께 줄일 수 있었을 것.
