...
카메라는 **비전이 소유**한다. 주행 코드는 카메라를 열지 않고, 비전이 tmpfs에 원자적으로
게시한 최신 결과를 **논블로킹으로 당겨 읽는다**(pull). 두 프로세스는 파일 하나로만 만난다.
...
**무중단 폴백이 설계의 핵심이다.** 비전 프로세스가 죽으면 게시가 낡고 → 신선도 판정이
invalid → 차선 보정은 0, 트리거는 미발화 → 주행은 GPS 단독 + 좌표 폴백으로 **계속된다**.
비전은 주행의 전제가 아니라 가산 신호다.
...
(`heading_error_deg`)를 받아 **바퀴각에 보정을 더한다**. 기저 조향은 항상 GPS 경로 추종이고,
비전은 그 위에 얹히는 제한된 보정이다 (명세서 §18-3: *보정값은 반드시 제한한다*).
...
> 오차로 추정)이 나와 `0.0` 으로 무력화한 적이 있고, **8/10 시연은 보정 0 상태로 완주했다.**
> 이 제출본은 원복값(30.0 / 0.3)으로 되돌려 두었으나 **그 값의 실주행 재검증은 하지 않았다.**
> 보정을 끄려면 두 게인을 `0.0` 으로 되돌린다 — 트리거는 게인과 무관하게 계속 동작한다.

---

## 3. 조향 트리거 (회전 개시 시점 결정)

차선 보조와 **같은 게시 파일을 읽지만 쓰임이 완전히 다른 두 번째 소비자**다.
회전을 "좌표로 언제 꺾을지" 대신 **"노면표시가 보이면 꺾는다"** 로 바꾼 것이 핵심이다.
비전 트리거의 가치는 정확도가 아니라 **좌표계 독립성**이다 — 8/7 실측에서 좌표계가 세 번
바뀌는 동안 `22->14` 트리거는 7/7 발화하며 매번 같은 지점에서 꺾었다.
...
| ③ | 상자 **하단** y ≥ 높이 × `near_row_frac` | 표식이 차에 그만큼 가까이 왔다 = **발화 시점 조절 손잡이** |
| ④ | 상자 중심의 횡거리 ≤ `max_lateral_m` | 남의 차선 표식 기각 (실측에서 정지선이 차선 5~6개 밖까지 잡혔다) |

`near_row_frac` 이 낮을수록 표식이 멀 때 발화 = **일찍 꺾는다.** 회전 튜닝은 이 값 하나로 한다.

**비전이 죽어도 회전은 한다** — 트리거가 영원히 안 뜨면 `fallback_at_lane_end` 로 차선 끝에서
좌표 단독 개시한다. 시연 로그의 `트리거 좌표단독` 이 그 경로다.
...
# ② 주행 — 비전 게시가 있어야 seg_mode:real 로 뜬다
cd rc_car
python3 main.py >> ~/veh_MMDD.log 2>&1
```

`vision_runner.py --stream-port 8090` 은 브라우저 실시간 확인용이며 **정차 중에만** 쓴다
(주행 중 무선 전송이 WiFi 동결을 유발한 사고가 있었다).
...
`seg_mode` 는 **config 한 줄 또는 `--seg-mode` 한 플래그**로 `off / mock / real` 이 갈린다.
`real` 은 비전 게시 파일이 없으면 **기동을 거부한다**(계약: 카메라 실패 = 기동 거부).
...
서버 주소·차량 토큰·개발 PC 경로는 `<...>` 플레이스홀더로 치환해 두었다. 실제로 돌리려면
`rc_car/config/network.yaml` 의 두 줄을 환경에 맞게 채운다.
...
1. **시연 당일 비전은 꺼져 있었다.** 완주 3회는 `--seg-mode off` (GPS 단독 + 좌표 폴백
   회전)로 냈다. 비전 경로는 8/5~8/8 실주행에서 검증된 코드이고 이 제출본에 전부 포함돼
   있으나, **8/10 완주 기록 자체는 비전 없이 낸 것이다.**
2. **차선 보정 게인은 재검증 전이다** (§2 경고 참조).
3. **단위 테스트는 전량 통과한다** — 383건 수집: 382 passed, 1 skipped(Windows에 없는
   SIGHUP), 서브테스트 462. 2026-08-10 제출 원본에서는 15건이 실패했었다 — 원인은 코드
   결함이 아니라 **버전 짝**(`rc_car` 는 8/8 스냅샷, `map` 은 8/9 갱신본)으로, 실차 튜닝으로
   바뀐 값을 테스트 말뚝이 따라가지 못한 것이다. 이 정리본은 말뚝을 시연 확정값
   (`turn_table` 말뚝, 도착 반경 0.40 정합)으로 갱신해 해소했다. **주행 동작에는 원래부터
   영향이 없었다** — 시연 3회 완주가 그 조합 그대로였다.
...
세부 실행·검증 절차는 `rc_car/README.md`, 맵 정의는 `map/README.md` 를 본다.
