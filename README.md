...
차선 보조와 같은 게시 파일을 읽지만 **기능이 완전히 다른 두 번째 모듈**.
회전 개시 기준을 좌표에서 노면표시로 바꿔, 표시가 화면 하단에 충분히 가까워지는
시점에 회전을 시작함. 좌표계가 바뀌어도 같은 지점에서 꺾는 것이 장점 — 8/7 실측에서
좌표계가 세 번 바뀌는 동안 `22->14` 트리거는 7/7 작동, 매번 같은 지점에서 회전 개시.
...
설정에 따라 차선 끝 좌표에서 회전을 시작함. 시연 로그의 `트리거 좌표단독` 이 이 경우의 기록.
